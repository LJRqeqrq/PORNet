...
class ResampleLoss(nn.Module):
    ...
    def __init__(
        ...
        freq_file=dict(class_freq=[1, 1], neg_class_freq=[1, 1]),
    ):
        ...
        self.class_freq = (
            torch.from_numpy(np.asarray(freq_file["class_freq"])).float().cuda()
        )
        self.neg_class_freq = (
            torch.from_numpy(np.asarray(freq_file["neg_class_freq"])).float().cuda()
        )
    ...
    def logit_reg_functions(self, labels, logits, weight=None):
        ...
        if "init_bias" in self.logit_reg:
            # 确保 init_bias 与 logits 在同一个设备上
            init_bias = self.init_bias.to(logits.device)
            logits += init_bias
    ...
    def rebalance_weight(self, gt_labels):
        # 确保 freq_inv 与 gt_labels 在同一个设备上
        freq_inv = self.freq_inv.to(gt_labels.device)
    ...
    def CB_weight(self, gt_labels):
        # 确保 class_freq 与 gt_labels 在同一个设备上
        class_freq = self.class_freq.to(gt_labels.device)

        if "by_class" in self.CB_mode:
            weight = torch.tensor((1 - self.CB_beta)).to(gt_labels.device) / (
                1 - torch.pow(self.CB_beta, class_freq)
            ).to(gt_labels.device)
        elif "average_n" in self.CB_mode:
            avg_n = torch.sum(gt_labels * class_freq, dim=1, keepdim=True) / torch.sum(
            ...
            weight_ = torch.tensor((1 - self.CB_beta)).to(gt_labels.device) / (
                1 - torch.pow(self.CB_beta, class_freq)
            ).to(gt_labels.device)
            ...
            min_n, _ = torch.min(
                gt_labels * class_freq + (1 - gt_labels) * 100000,
                dim=1,
                keepdim=True,
            )
            weight = torch.tensor((1 - self.CB_beta)).to(gt_labels.device) / (
    ...
    def RW_weight(self, gt_labels, by_class=True):
        # 确保 propotion_inv 与 gt_labels 在同一个设备上
        propotion_inv = self.propotion_inv.to(gt_labels.device)

        if "sqrt" in self.reweight_func:
            weight = torch.sqrt(propotion_inv)
        else:
            weight = propotion_inv
        if not by_class:
